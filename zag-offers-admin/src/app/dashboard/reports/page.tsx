...
function ErrorPanel({ message }: { message: string }) {
  return (
    <div className="admin-panel p-10 text-center">
      ...
      <h3 className="text-lg font-black text-slate-900 mb-2">تعذر تحميل التقارير</h3>
  ...
  if (statsError) {
    return (
      <div className="p-6 lg:p-10">
        <ErrorPanel message={(statsErrorData as Error)?.message || 'حدث خطأ غير متوقع أثناء تحميل البيانات.'} />
      </div>
    );
  }
    ...
    <div className="p-6 lg:p-10 space-y-8">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <PageHeader
          ...
          description={`متابعة أداء المنصة بشكل تفصيلي - ${periodLabel}`}
          icon={BarChart3}
        />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="admin-panel p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-black text-slate-900">أفضل المتاجر أداءً</h3>
            ...
            <div className="space-y-3">
              {topStores.slice(0, 5).map((store, index) => (
                <div key={store.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3">
                  <div className="h-8 w-8 rounded-lg bg-orange-100 text-orange-700 text-xs font-black flex items-center justify-center">
                    {index + 1}
                  </div>
                  ...
                  <div className="text-left">
                    <p className="text-sm font-black text-slate-900 tabular-nums">{store._count?.offers ?? 0}</p>
                    <p className="text-[10px] text-slate-500">عروض</p>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-slate-500">لا توجد بيانات متاجر متاحة حالياً.</p>
          )}
        </div>

        <div className="admin-panel p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-sm font-black text-slate-900">أفضل التصنيفات</h3>
            ...
            <div className="space-y-3">
              {topCategories.slice(0, 5).map((category, index) => (
                <div key={category.name} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3">
                  <div className="h-8 w-8 rounded-lg bg-emerald-100 text-emerald-700 text-xs font-black flex items-center justify-center">
                    {index + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="text-left">
                    <p className="text-sm font-black text-slate-900 tabular-nums">{category._count?.stores ?? 0}</p>
                    <p className="text-[10px] text-slate-500">متجر</p>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-slate-500">لا توجد بيانات تصنيفات متاحة حالياً.</p>
          )}
        </div>
      </div>

      <div className="admin-panel p-6">
        <h3 className="text-sm font-black text-slate-900 mb-5">مؤشرات تفصيلية</h3>
        ...
        {statsLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              ...
              <p className="text-2xl font-black text-slate-900 tabular-nums">{stats?.users.totalUsers ?? 0}</p>
            </div>
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <p className="text-[11px] font-bold text-slate-500 mb-1">إجمالي المتاجر</p>
              <p className="text-2xl font-black text-slate-900 tabular-nums">{stats?.stores.totalStores ?? 0}</p>
            </div>
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <p className="text-[11px] font-bold text-slate-500 mb-1">إجمالي العروض</p>
              <p className="text-2xl font-black text-slate-900 tabular-nums">{stats?.offers.totalOffers ?? 0}</p>
            </div>
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <p className="text-[11px] font-bold text-slate-500 mb-1">معدل التحويل</p>
              <p className="text-2xl font-black text-slate-900 tabular-nums">{stats?.coupons.couponConversionRate ?? '0%'}</p>
            </div>
          </div>
        )}
      </div>
