...
import { LayoutDashboard, Users, Store, Settings, Bell } from 'lucide-react';
import { motion } from 'framer-motion';

const navItems = [
  { icon: LayoutDashboard, label: 'الرئيسية', path: '/dashboard' },
  { icon: Store, label: 'التجار', path: '/dashboard/merchants' },
...
export default function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[60] px-4 pb-6 pt-2">
      <div className="glass rounded-[2rem] border border-white/5 bg-slate-900/80 backdrop-blur-2xl shadow-2xl flex items-center justify-around p-2">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`relative z-10 transition-all ${isActive ? 'text-primary scale-110' : 'text-slate-400 hover:text-white'}`}>
