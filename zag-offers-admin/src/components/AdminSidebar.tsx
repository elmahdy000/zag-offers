    ...
    <div className="flex h-full flex-col bg-white border-l border-slate-200">
      ...
      <div className="flex items-center gap-4 px-8 py-10">
        ...
        <div className="flex flex-col">
          <span className="text-xl font-bold text-slate-900 tracking-tight leading-none">ZAG <span className="text-orange-600">OFFERS</span></span>
          <span className="text-[10px] font-bold text-slate-400 mt-1.5 uppercase tracking-[0.2em]">إدارة المنصة المركزية</span>
        </div>
      </div>
      ...
      <nav className="flex-1 space-y-1 px-5 overflow-y-auto custom-scrollbar pb-6">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon size={18} className={active ? 'text-white' : 'text-slate-400 group-hover:text-orange-600 transition-colors'} />
              <span className="flex-1">{item.name}</span>
              {item.href === '/dashboard/approvals' && pendingCount > 0 && (
                <span className={`flex h-5 min-w-[20px] items-center justify-center rounded-lg px-1.5 text-[10px] font-bold shadow-sm ${active ? 'bg-white/20 text-white' : 'bg-orange-100 text-orange-600'}`}>
                  {pendingCount}
                </span>
              )}
            </Link>
      ...
      <div className="mt-auto border-t border-slate-100 p-6 space-y-3">
        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-[9px] font-medium text-slate-400">الإعدادات العامة</p>
          </div>
        </div>
        <button
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsOpen(false)} className="fixed inset-0 z-40 bg-slate-950/20 backdrop-blur-sm lg:hidden" />
            <motion.aside
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              exit={{ x: '100%' }}
            ...
            >
              <SidebarContent />
              <button onClick={() => setIsOpen(false)} className="absolute left-[-56px] top-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-slate-900 shadow-2xl border border-slate-100">
                <X size={24} />
              </button>
            </motion.aside>
          </>
