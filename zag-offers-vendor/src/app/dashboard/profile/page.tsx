...
export default function StoreProfilePage() {
  ...
  if (isError) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-bg text-center">
        <div className="w-20 h-20 bg-red-500/10 rounded-3xl flex items-center justify-center text-red-500 mb-6 border border-red-500/20 shadow-xl">
          <AlertCircle size={40} />
        </div>
        <h2 className="text-xl font-black text-text mb-2">خطأ في تحميل البيانات</h2>
        <p className="text-text-dim text-sm max-w-sm mb-8 leading-relaxed">
          {errorStatus === 404 
  ...
  return (
    <div className="p-4 sm:p-8 lg:p-12 dir-rtl bg-bg min-h-screen max-w-7xl mx-auto">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-8">
          {/* Logo Card */}
          <div className="glass p-8 rounded-[2.5rem] border border-white/10 flex flex-col items-center relative overflow-hidden">
             <div className="absolute top-0 right-0 p-4 opacity-10">
                <Store size={80} />
             </div>
             
             <div className="relative mb-6">
                <div className="w-36 h-36 bg-bg2 border-4 border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl relative">
                  {store.logo ? (
                ...
                <button onClick={() => document.getElementById('logo-upload')?.click()} className="absolute -bottom-2 -left-2 bg-primary text-white p-3.5 rounded-2xl shadow-xl hover:scale-110 transition-all border-4 border-bg">
                  <Camera size={20} />
                </button>
             </div>

             <div className="text-center">
               <h3 className="text-lg font-black text-text mb-1">{store.name}</h3>
          ...
          {/* Verification Status */}
          <div className={`p-6 rounded-[2rem] border ${store.status === 'APPROVED' ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-amber-500/5 border-amber-500/10'}`}>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="glass p-8 sm:p-10 rounded-[2.5rem] border border-white/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-text-dim mr-1">اسم المتجر</label>
                <input 
                  type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })}
                  className="w-full bg-bg2 border border-white/5 rounded-2xl py-4 px-5 text-sm font-bold text-text focus:border-primary outline-none transition-all"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-text-dim mr-1">رقم الهاتف</label>
                <input 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-text-dim mr-1">واتساب (للكوبونات)</label>
                <input 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-text-dim mr-1">رابط الخريطة (Google Maps)</label>
                <div className="relative">
                  <input 
                    type="text" value={formData.locationUrl} onChange={e => setFormData({ ...formData, locationUrl: e.target.value })}
                    className="w-full bg-bg2 border border-white/5 rounded-2xl py-4 px-5 pl-12 text-sm font-bold text-text focus:border-primary outline-none transition-all"
                  />
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-[10px] font-black text-text-dim mr-1">العنوان بالتفصيل</label>
                <textarea 
                  value={formData.address} onChange={e => setFormData({ ...formData, address: e.target.value })}
                  className="w-full bg-bg2 border border-white/5 rounded-2xl py-4 px-5 text-sm font-bold text-text focus:border-primary outline-none transition-all min-h-[100px] resize-none"
                />
              </div>
            </div>
          </div>
          ...
          <div className="glass p-8 sm:p-10 rounded-[2.5rem] border border-white/10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {(store.images || []).map((img: string, idx: number) => (
                <div key={idx} className="aspect-square rounded-3xl overflow-hidden border border-white/5 relative group">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button onClick={() => setImageToDelete(idx)} className="bg-red-500 text-white p-3 rounded-2xl shadow-lg">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
              ))}
              <button onClick={() => document.getElementById('gallery-upload')?.click()} className="aspect-square rounded-3xl border-2 border-dashed border-white/10 hover:border-primary/50 hover:bg-primary/5 transition-all flex flex-col items-center justify-center gap-2 text-text-dimmer">
                 <Camera size={24} />
                 <span className="text-[9px] font-black uppercase">أضف صورة</span>
              </button>
            </div>
          </div>
        </div>
        ...
        {success && (
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
            <div className="bg-emerald-500 text-white px-8 py-4 rounded-3xl shadow-2xl flex items-center gap-3">
              ...
              <span className="text-sm font-black">تم تحديث البيانات بنجاح</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
