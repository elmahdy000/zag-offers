...
import { 
  Store, MapPin, Phone, Camera, Save, Loader2, 
  CheckCircle2, MessageCircle, Trash2, Plus, 
...
export default function StoreProfilePage() {
  ...
  useEffect(() => {
    if (store) {
      const timer = window.setTimeout(() => setFormData({
        name: store.name || '',
        phone: store.phone || '',
  ...
  if (isError) {
    ...
    return (
      <div className="vendor-profile-state-page">
        <EmptyState
          icon={<AlertCircle size={30} />}
          title={errorStatus === 404 ? 'الحساب غير مرتبط بمتجر' : 'تعذر تحميل بيانات المتجر'}
          description={errorStatus === 404
            ? 'لم نعثر على متجر مرتبط بهذا الحساب. إذا تم تسجيلك حديثًا، تواصل مع الدعم لإكمال الربط.'
            : 'تعذر الاتصال بالخادم. تحقق من الإنترنت ثم أعد المحاولة.'}
          actionText="إعادة المحاولة"
          actionIcon={<RefreshCw size={16} />}
          onAction={() => refetch()}
          secondaryText="التواصل مع الدعم"
  ...
  if (!store) {
    return (
      <div className="vendor-profile-state-page">
        <EmptyState
          icon={<Store size={30} />}
          title="لم يتم ربط بيانات المتجر بعد"
          description="بيانات متجرك غير متاحة لهذا الحساب حاليًا. أعد المحاولة أو تواصل مع الدعم لإتمام عملية الربط."
          actionText="إعادة المحاولة"
          actionIcon={<RefreshCw size={16} />}
          onAction={() => refetch()}
          secondaryText="التواصل مع الدعم"
  ...
  return (
    <div className="vendor-profile-page p-4 sm:p-8 lg:p-12 dir-rtl bg-bg min-h-screen max-w-7xl mx-auto animate-in">
      {/* Header Section */}
      <div className="vendor-profile-header flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-7">
      ...
      <div className="vendor-profile-grid grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Profile Sidebar */}
        <div className="space-y-8">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="vendor-profile-summary glass p-7 rounded-[2.5rem] border border-glass-border flex flex-col items-center relative overflow-hidden bg-bg2/40">
             <div className="relative mb-6">
                <div className="w-36 h-36 bg-bg2 border-4 border-glass-border rounded-[2.5rem] overflow-hidden shadow-2xl relative group">
                  {store.logo ? (
                    <img src={resolveImageUrl(store.logo)} alt={`شعار ${formData.name || 'المتجر'}`} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                  ) : (
        ...
        <div className="lg:col-span-2 space-y-8">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }} className="vendor-profile-panel glass p-6 sm:p-8 rounded-[2.5rem] border border-glass-border shadow-xl bg-bg2/20">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  {formData.locationUrl && (
                    <a href={formData.locationUrl} target="_blank" rel="noreferrer" className="absolute left-4 top-1/2 -translate-y-1/2 text-primary hover:scale-110 transition-transform">
          ...
          {/* Photo Gallery Card */}
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="vendor-profile-panel glass p-6 sm:p-8 rounded-[2.5rem] border border-glass-border shadow-xl bg-bg2/20">
              ...
              <input type="file" id="gallery-upload" className="hidden" accept="image/*" multiple onChange={async (e) => {
                ...
                for (const file of files) {
                  ...
                  try {
                    ...
                    addError('تعذر رفع إحدى الصور. تحقق من الملف وحاول مرة أخرى.');
                  }
                }
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {(store.images || []).map((img: string, idx: number) => (
                <div key={idx} className="aspect-square rounded-3xl overflow-hidden border border-glass-border relative group bg-bg shadow-lg">
                  <img src={resolveImageUrl(img)} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt={`صورة من معرض المتجر ${idx + 1}`} />
